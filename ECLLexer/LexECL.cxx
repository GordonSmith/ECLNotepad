...
static void FoldEclDoc(unsigned int startPos, int length, int initStyle, 
	...
	for (unsigned int i = startPos; i < endPos; i++) {
		...
		if (style == SCE_ECL_WORD0 || style == SCE_ECL_WORD1 || style == SCE_ECL_WORD2 || style == SCE_ECL_WORD3 || style == SCE_ECL_WORD4 || style == SCE_ECL_WORD5) {
			if (styler.Match(i, "endmacro"))
			{
				int d = 0;//Good breakpoint for working out what style somthing is.
			}
		}
...
extern LexerModule lmBatch, lmDiff, lmPo, lmProps, lmMake, lmErrorList, lmLatex, lmNull;
...
const LexerModule *Catalogue::Find(const char *languageName) {
	if (strcmp(languageName, "ecl") == 0) 
		return &lmECL;
	else if (strcmp(languageName, "batch") == 0) 
		return &lmBatch;
	else if (strcmp(languageName, "diff") == 0) 
		return &lmDiff;
	else if (strcmp(languageName, "po") == 0) 
		return &lmPo;
	else if (strcmp(languageName, "props") == 0) 
		return &lmProps;
	else if (strcmp(languageName, "makefile") == 0) 
		return &lmMake;
	else if (strcmp(languageName, "errorlist") == 0) 
		return &lmErrorList;
	else if (strcmp(languageName, "latex") == 0) 
		return &lmLatex;
	else if (strcmp(languageName, "null") == 0) 
		return &lmNull;
	return NULL;
}
